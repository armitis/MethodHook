...
//            }
//            int index = dtp_record_num - 1;
            if (dtp_record_index >= 10240) {
                dtp_record_index = 0;
            }
            dtp_call_record *log = &dtp_records[dtp_record_index];
            log->cls = pRecord->cls;
            //cls在获取的时候会变成野指针，这里通过sel存储cls名称，后面再通过sel获取
            const char* name = class_getName(pRecord->cls);
            log->clsName = sel_registerName(name);
            log->dirty = 1;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
//    });
    pthread_key_create(&_thread_key, &release_thread_call_stack);
    fm_rebind_symbols((struct fm_rebinding[1]){"objc_msgSend", (void *)hook_objc_msgSend, (void **)&orig_objc_msgSend},1);
}

void dtp_hook_end(void) {
    _call_record_enabled = false;
    fm_rebind_symbols((struct fm_rebinding[1]){"objc_msgSend", (void *)orig_objc_msgSend, NULL},1);
}
...
dtp_call_record *dtp_get_call_records(int *num) {
    if (num) {
        *num = dtp_record_alloc;
    }
    return dtp_records;
}

int dtp_get_current_index(void) {
    if (dtp_records) {
        return dtp_record_index;
    }
    return -1;
}
